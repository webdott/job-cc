...
| `GOOGLE_GENERATIVE_AI_KEY`          | [aistudio.google.com](https://aistudio.google.com) → Get API key      |
...
- **AI layer** — `lib/ai.ts` exports `proModel` (Gemini 1.5 Pro) and `flashModel` (Flash). Swap to Claude by changing two lines
...
- **Push** — VAPID keys + `web-push`. Subscriptions stored in `PushSubscription` table
